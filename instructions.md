...
6. *(Optional)* Run **Login to StartOS** to authenticate the bundled `start-cli` so the agent can administer this server. It asks for your StartOS master password, uses it for that login action, and stores a `start-cli` auth cookie on Hermes' data volume. **This grants the agent root-equivalent access — only do this on a machine you treat as expendable.**
